...
func main() {
	...
	router.HandleFunc("/driver/driverStatus", statusDriverEndpoint).Methods("PATCH")
...
func statusDriverEndpoint(w http.ResponseWriter, r *http.Request) {
	if r.Method == "PATCH"{
		querystringmap := r.URL.Query()
		...
		//inserting values into passenger table
		_, err = db.Query("UPDATE driver SET isAvail=1 where driverId = " + driverId)
		//Handling error of SQL statement
		if err != nil {
			http.Error(w, "Missing data", http.StatusBadRequest)
		} else {
