...
func TripStatus(driverId int, isAvail bool) {
	...
	if isAvail == true {
		...
		if tripDetails.IsCompleted == false && tripDetails.IsStarted == false && tripDetails.TripId != 0{
		...
		if tripDetails.IsCompleted == false && tripDetails.IsStarted != false{
			...
			switch choice {
			case "Y":
				endTrip(tripDetails.TripId, driverId)
				break;
			case "N":
				break;
			default:
...
func startTrip(tripId int){
	client := &http.Client{}
	//sending is started is false to api call
...
func endTrip(tripId int, driverId int){
	client := &http.Client{}
	...
	if req, err := http.NewRequest("PATCH", url, resBody); err == nil {
		if res, err2 := client.Do(req); err2 == nil {
			if res.StatusCode == 202 {
				...
				client := &http.Client{}
				url = "http://localhost:5000/driver/driverStatus?driverId="+ strconv.Itoa(driverId)
				postBody, _ := json.Marshal(tripId)
				resBody := bytes.NewBuffer(postBody)
				if req, err := http.NewRequest("PATCH", url, resBody); err == nil {
					if res, err2 := client.Do(req); err2 == nil {
